...
SAFETY_V2_JSON = json.dumps(
    [
        [
            ...
            "<2.28.0",
            "2.27.0",
            "CVE-2023-32681: Unintended leak.",
            "CVE-2023-32681",
        ],
    ]
)
...
class TestRunSafetyDispatcher:
    ...
    @patch("bot.scanner._tool_available", return_value=True)
    @patch(
        "bot.scanner._run_pip_audit", return_value=[make_finding(scanner="pip-audit")]
    )
...
class TestFormatComment:
    ...
    def test_overall_line_present(self):
        result = ScanResult(
            bandit_findings=[make_finding(severity="MEDIUM")], coverage_pct=90.0
        )
        assert "Overall:" in format_comment(result, "abc123f")

    def test_fix_summary_shown_when_applied(self):
        comment = format_comment(
            ScanResult(), "abc123f", fix_summary={"black": True, "autoflake": True}
        )
    ...
    def test_flake8_capped_at_10(self):
        findings = [
            make_finding(
                scanner="flake8", rule_id="E501", severity="MEDIUM", title=f"issue {i}"
            )
            for i in range(15)
        ]
    ...
    def test_cve_findings_shown(self):
        result = ScanResult(
            cve_findings=[make_finding(scanner="pip-audit", rule_id="CVE-2024-1234")]
        )
